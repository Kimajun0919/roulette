...
- `docs/FIGMA_STRICT_IMPORT_CONTRACT.md`
  - "이 안에서 만들면 바로 import" 기준의 엄격 계약서
- `docs/FIGMA_DESIGN_REQUEST.md`
  - 디자이너 전달용 요청서
- `docs/examples/figma-wrapper-payload.example.json`
  - wrapper payload handoff 예시
...
- effect parity 100%
  - `INNER_SHADOW`, `BACKGROUND_BLUR`는 1차 오프스크린 합성 구현 완료
  - 하지만 복합 blend, 고급 mask, 브라우저 차이까지 포함한 완전한 Figma parity는 아직 아님
- Figma export 자동화
...
- 고급 mask mode 세부 규칙 확장
- effect parity 보정
  - `INNER_SHADOW` 품질 보정
  - `BACKGROUND_BLUR` 품질 보정
- 복잡한 vector/path -> collider 확장
